--[[
    YSL Bá Sàn v16.1 – Tối ưu cho Mobile & iOS
    - Auto Fire sử dụng Tool:Activate() không làm khựng cảm ứng.
    - Sẵn sàng để upload lên GitHub (Loadstring).
    - Cải thiện hiệu suất vòng lặp RenderStepped.
]]
...
local values = {
    ...
    autoFireInterval = 0.1, -- Giảm xuống một chút để bắn mượt hơn
...
local function isVisible(targetRoot)
    ...
    -- Tối ưu: Chỉ kiểm tra ngực (Torso/RootPart) để giảm chi phí tính toán raycast
    local raycastParams = RaycastParams.new()
    raycastParams.FilterType = Enum.RaycastFilterType.Exclude
    raycastParams.FilterDescendantsInstances = {myChar, camera}
    
    local direction = (targetRoot.Position - origin).Unit * values.aimbotRange
...
-- 캐시 lại mục tiêu gần nhất để dùng chung cho Aimbot và AutoFire (Tránh tính toán 2 lần mỗi frame)
...
local function updateNearestEnemy()
    ...
    currentAimbotTarget = bestRoot
end

-- Auto Fire tối ưu cho Mobile (Không dùng VirtualInputManager)
local function attack()
    local char = getCharacter(player)
    if not char then return end
    
    -- Ưu tiên tìm Tool đang cầm trên tay
            ...
            if eRoot then
                ...
                if dist <= values.espMaxDistance then
                    if not espData[p] then
                        ...
                        billboard.Adornee = eChar:FindFirstChild("Head") or eRoot 
...
addConnection(RunService.RenderStepped:Connect(function()
    -- Cập nhật mục tiêu liên tục 1 lần duy nhất cho cả Aimbot và AutoFire
...
-- Vòng lặp bắn súng độc lập, tránh bị đè frame
...
print("[YSL Bá Sàn v16.1] Sẵn sàng để up GitHub - AutoFire không khựng Mobile.")
